...
const LEGAL_TITLE_EN = {
  'section-barrier-divider-hard-law': 'International legal framework',
  'outer-space': 'Outer Space Treaty',
  liability: 'Liability Convention',
  registration: 'Registration Convention',
  'other-treaties': 'Other space treaties and principles',
  'section-barrier-divider-soft-law': 'Soft law and technical standards',
  mitigation: 'COPUOS Space Debris Mitigation Guidelines',
  'lts-guidelines': 'Long-term Sustainability Guidelines',
  'iadc-guidelines': 'IADC Space Debris Mitigation Guidelines',
  'iso-24113': 'ISO 24113:2023',
  'esa-zero-debris': 'ESA Space Debris Mitigation Policy',
  'section-barrier-divider-national': 'National and regional regulation',
  'us-fcc': 'United States: FCC and federal practice',
  'eu-space-act': 'European Union: EU Space Act',
  'france-fsoa': 'France: Space Operations Authorization',
  'uk-caa': 'United Kingdom: Spaceflight licensing',
  'china-standards': 'China: licensing, registration, and safety',
  'japan-space-activities': 'Japan: Space Activities Act and licensing',
  'section-barrier-divider-gaps': 'Major governance gaps',
  overview: 'Rules exist, but governance remains fragmented',
  'gap-no-treaty': 'No dedicated binding global convention',
  'gap-soft-law': 'Voluntary rules depend on adoption',
  'gap-standards': 'Technical standards remain inconsistent',
  'gap-liability': 'Attribution and liability remain difficult',
  'gap-legacy': 'Legacy debris has no clear cleanup duty',
  'gap-consent': 'Ownership and consent obstruct removal',
  'gap-traffic': 'No unified space-traffic authority',
  'section-barrier-divider-expressions': 'Visual and interaction notes',
  'display-main-copy': 'Narrative focus',
  'display-model-copy': 'System model',
  'display-interaction-copy': 'Archive interaction',
  'section-barrier-divider-sources': 'Sources and archive index',
  'source-index': 'Source index',
}

const LEGAL_SECTION_EN = {
  1: 'INTERNATIONAL LAW',
...
function localizeLegalDocument(document, language) {
  ...
  const title = LEGAL_TITLE_EN[document.id] || document.source || `Legal dossier ${document.displayLabel || ''}`.trim()
  const isBarrier = document.kind === 'section-barrier'

  return {
    ...document,
    title,
    section: LEGAL_SECTION_EN[document.sectionIndex] || 'LEGAL ARCHIVE',
    tag: isBarrier ? 'SECTION DIVIDER' : LEGAL_SECTION_EN[document.sectionIndex] || 'LEGAL RECORD',
    summary: isBarrier
      ? `This section groups records concerning ${title.toLowerCase()}.`
      : `This dossier summarizes ${title} and its relevance to responsibility, debris mitigation, licensing, or enforcement in outer space.`,
    points: isBarrier ? [] : [
      'Defines or supports a part of the current space-governance framework.',
      'Influences mission design, operation, registration, licensing, or disposal.',
      'Its practical effect depends on jurisdiction, adoption, and enforcement.',
    ],
    limit: isBarrier
      ? `The records below show both existing rules and the gaps that remain.`
      : 'Its scope or enforcement remains limited and does not create a single mandatory global debris-cleanup regime.',
  }
}
...
export default function LegalTreaties({ onComplete = () => {} }) {
  ...
  const localizedFlowText = localizedDocuments
    ...
    .map((item) => item.title)
    .join('  ·  ')
  const localizedFlowMarquee = language === 'en'
    ? `${localizedFlowText}  ·  ${localizedFlowText}  ·  `
    ...
    >
      ...
      <div className="law-stage">
        ...
        >
          ...
          <h2 aria-label={pick('责任失重｜太空垃圾的法律盲区', 'Responsibility in zero gravity: the legal blind spot of space debris')}>
            <span className="law-title-line">{pick('责任失重｜', 'RESPONSIBILITY IN ZERO GRAVITY')}</span>
            {' '}
            <span className="law-title-line">{pick('太空垃圾的法律盲区', 'THE LEGAL BLIND SPOT OF SPACE DEBRIS')}</span>
          </h2>
          <p>
            {pick(
              '现有国际空间法虽然规定了发射国的责任，但对于碎片追踪、责任认定、主动清理和惩罚机制仍不完善。太空垃圾因此成为一种“有归属却难追责”的全球性问题，也暴露出人类进入太空后尚未建立完整治理秩序的现实。',
              'International space law assigns responsibility to launching states, yet tracking, attribution, active removal, and enforcement remain incomplete. Space debris is therefore owned but difficult to govern, exposing the absence of a complete order for human activity in orbit.',
            )}
          </p>
        </motion.div>
