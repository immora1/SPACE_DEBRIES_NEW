...
const initialState = {
  ...
  storyLocalProgress: null,
  storyBackgroundPending: 0,
  storyBackgroundError: null,
...
const useAppStore = create(
  persist(
    (set, get) => ({
      ...
      setStoryLocalProgress: (progress) => set({
        storyLocalProgress: progress,
        publicGameState: progress.gameState,
        storyCheckpoint: progress.checkpoint,
      }),
      setStoryBackground: ({ jobs, error, running }) => set({
        storyBackgroundPending: jobs.length,
        storyBackgroundError: error,
        storyLoading: running && jobs.length > 0,
      }),
      startStoryRequest: () => set({ storyLoading: true, storyError: null }),
      setStoryError: (storyError) => set({ storyError, storyLoading: false }),
      setStorySnapshot: (story) => set((state) => ({
        storyId: story.story_id,
        storyVersion: story.version,
        storyStatus: story.status,
        storyCheckpoint: state.storyLocalProgress?.checkpoint || story.current_checkpoint,
        ...
        publicGameState: state.storyLocalProgress?.gameState || story.public_game_state || null,
        storyLoading: false,
        storyError: null,
        storySessionReady: true,
      })),
